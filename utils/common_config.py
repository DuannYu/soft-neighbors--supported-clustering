...
def get_model(p, pretrain_path=None):
    ...
    if p['backbone'] == 'resnet18':
        ...
        elif p['train_db_name'] in ['imagenet_50', 'imagenet_100', 'imagenet_200', 'imagenetdogs']:
            from models.resnet_stl import resnet18
            backbone = resnet18()
        else:
            raise NotImplementedError
...
def get_train_dataset(p, transform, to_augmented_dataset=False,
    ...
    if p['train_db_name'] == 'cifar-10':
    ...
    elif p['train_db_name'] in ['imagenet10', 'imagenetdogs']:
        from data.imagenet import ImageNetSubset, ImageFolder
        ...
        dataset = ImageNetSubset(
            root = '/home/duanyu/deep/DATASETS/imagenet_subsets/'+p['train_db_name'],
            subset_file='/home/duanyu/deep/contrastive/Soft-Neighbors/data/imagenet_subsets/'+p['train_db_name']+'.txt',
            transform=transform
        )
        
    elif p['train_db_name'] == 'tiny-imagenet-200':
        from data.imagenet import ImageNetSubset, ImageFolder
        # subset_file = './data/imagenet_subsets/%s.txt' %(p['train_db_name'])
        # dataset = ImageNetSubset(subset_file=subset_file, split='train', transform=transform)
        dataset = ImageNetSubset(
            root = '/home/duanyu/deep/DATASETS/imagenet_subsets/tiny-imagenet-200/train/',
            subset_file='/home/duanyu/deep/contrastive/Soft-Neighbors/data/imagenet_subsets/imagenet_200.txt',
            transform=transform
        )
    else:
...
def get_val_dataset(p, transform=None, to_neighbors_dataset=False):
    ...
    elif p['val_db_name'] in ['imagenet10', 'imagenet_100', 'imagenet_200', 'imagenetdogs']:
