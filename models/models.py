...
class ClusteringModel(nn.Module):
    ...
    @torch.no_grad()
    def update_prototypes(self, features, out, gamma=0.8):
        # new batch clustering center using fuzzy k-means
    ...
    def forward(self, x, forward_pass='default'):
        if forward_pass == 'default':
            ...
            self.update_prototypes(features, out['output'][0])

        else:
            raise ValueError('Invalid forward pass {}'.format(forward_pass))        

        # for forward inner product
        # similarity = torch.mm(features, self.prototypes.t())
        # pseudo_labels = torch.softmax(similarity, dim=1)
        return out
