...
def main():
    ...
    #TODO dataset = get_train_dataset(config, transforms)
    ...
    if config['setup'] in ['simclr', 'moco']:
        ...
        # import numpy as np
        # import os
        # epoch = args.model.split('/')[-1].split('_')[0]
        # save_path = './results/tsne_results/'
        
        # np.save(os.path.join(save_path, str(epoch)+'_labels.npy'), predictions[0]['predictions'])
        # np.save(os.path.join(save_path, str(epoch)+'_features.npy'), features.numpy())
        # np.save(os.path.join(save_path, str(epoch)+'_performance.npy'), clustering_stats)
